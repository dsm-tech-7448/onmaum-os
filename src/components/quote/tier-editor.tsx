...
import { resizeImageToDataUrl } from "@/lib/images/resize-image";
    ...
    <div>
      ...
      {tiers.map((tier, ti) => {
        ...
        function handleImageUpload(uid: string, file: File | undefined) {
          if (!file) return;
          resizeImageToDataUrl(file).then((imageDataUrl) => update(uid, "imageDataUrl", imageDataUrl));
        }
        ...
        return (
          <div key={tier.uid} className="mb-2.5 rounded-md border border-[#e2c5cf] bg-[#fdf7f9] p-2.5">
            ...
            <div
              ...
              style={{ gridTemplateColumns: "44px 1fr 46px 55px 62px 60px", minWidth: "max-content" }}
            >
              <div>구분</div>
              <div>품명/옵션명</div>
              <div>단위</div>
