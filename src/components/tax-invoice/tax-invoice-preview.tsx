...
function calc(items: TaxInvoiceDraft["items"]) {
  ...
  return { merged, total: merged.supply + merged.tax };
}
...
export function TaxInvoicePreview({
  ...
  const { merged, total } = calc(draft.items);
    ...
    <div className="tax-invoice-doc">
      ...
      <table className="info-table">
        <tbody>
          <tr>
            ...
            <td className="k">작성일자</td>
            <td>{draft.scheduledDate}</td>
          </tr>
          <tr>
            <td className="k">영수/청구</td>
            <td colSpan={3}>{draft.purposeType === "영수" ? "영수" : "청구"}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>등록번호</td>
            <td colSpan={2}>
              {draft.customerBusinessNumber || "-"}
              {draft.customerSubNum ? ` (종사업장 ${draft.customerSubNum})` : ""}
            </td>
          </tr>
          <tr>
            <td>상호</td>
      ...
      <table className="items">
        ...
        <tbody>
          {merged.name && (
            <tr>
              <td className="itemname">{merged.name}</td>
              <td>{merged.qty ? Number(merged.qty).toLocaleString() : "-"}</td>
              <td>{merged.supply ? fmt(merged.supply) : "-"}</td>
              <td>{merged.tax ? fmt(merged.tax) : "-"}</td>
            </tr>
          )}
          <tr className="total-row">
            <td colSpan={2}>합계 (공급가액 {fmt(merged.supply)} + 세액 {fmt(merged.tax)})</td>
