...
import { db } from "@/db";
import { summaryOverrides, quoteItems } from "@/db/schema";
...
// 고객이 요청한 납품 희망일 — 발주서의 발송요청일(공급업체용)과 무관한 별개 값이라
// summary_overrides.cust_req_date에 저장한다. 견적 하나에 품목이 여러 개면(한 공급업체로
// 묶은 발주서의 출금을 합쳐서 관리하는 것과 같은 이유로, 2026-09-15) 그 견적의 모든 품목
// 행에 같은 값을 한 번에 반영한다 — 품목마다 따로 입력할 필요 없게.
export async function updateCustomerReqDate(quoteId: string, custReqDate: string): Promise<void> {
  const session = await requireSession();

  const rows = await db.select({ id: quoteItems.id }).from(quoteItems).where(eq(quoteItems.quoteId, quoteId));
  const value = custReqDate.trim() || null;

  for (const row of rows) {
    await db
      .insert(summaryOverrides)
      .values({ rowKey: row.id, custReqDate: value, updatedBy: session.userId })
      .onConflictDoUpdate({
        target: summaryOverrides.rowKey,
        set: { custReqDate: value, updatedAt: new Date(), updatedBy: session.userId },
      });
  }
}
