...
export function PoEditor({
  ...
  async function handleExportPng() {
    ...
    try {
      ...
      const productName = safe(draft.items[0]?.name || "품목");
      // 리비전 번호를 파일명 끝에 붙인다(2026-09-18) — 같은 발주서를 리비전으로 여러 번
      // 다시 저장하면 파일명이 같아서 어느 버전인지 구분이 안 됐다.
      const revLabel = `Rev${String(revision || 1).padStart(2, "0")}`;
      a.href = url;
      a.download = `발주서-${safe(draft.supplierName || "공급업체")}_${productName}_${revLabel}.png`;
