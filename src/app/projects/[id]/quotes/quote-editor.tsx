...
export function QuoteEditor({
  ...
  const [customerInfoHint, setCustomerInfoHint] = useState("");

  const [searchQuery, setSearchQuery] = useState("");
  const [quoteList, setQuoteList] = useState<QuoteSearchResult[]>(initialQuoteList);
  const [deletingQuoteNumber, setDeletingQuoteNumber] = useState<string | null>(null);
  ...
  // 고객사명/담당자/전화만 수정 저장 — 새 리비전을 만들지 않고 지금 불러온 리비전 행을
  // 그대로 고친다(2026-09-22 피드백: 담당자가 바뀌는 등 문서 내용은 그대로인데 연락처만
  // 정정되는 경우가 있어 매번 리비전이 올라가지 않게 해달라는 요청).
  async function handleSaveCustomerInfo() {
    if (!draft.id) return;
    setIsSavingCustomerInfo(true);
    setCustomerInfoHint("");
    try {
      await updateQuoteCustomerInfo(projectId, draft.id, {
        customerName: draft.customerName,
        contactName: draft.contactName,
        contactPhone: draft.contactPhone,
      });
      setCustomerInfoHint("고객 정보가 저장되었습니다 (리비전 생성 없음).");
      const list = await searchProjectQuotes(projectId, searchQuery);
      setQuoteList(list);
      router.refresh();
    } catch (err) {
      setCustomerInfoHint(err instanceof Error ? err.message : "저장에 실패했습니다.");
    } finally {
      setIsSavingCustomerInfo(false);
    }
  }

  async function handleDeleteQuote(quoteNumber: string) {
    if (!window.confirm(`${quoteNumber}(모든 리비전)를 삭제할까요? 되돌릴 수 없습니다.`)) return;
    setDeleteError("");
    setDeletingQuoteNumber(quoteNumber);
    try {
      await deleteQuote(projectId, quoteNumber);
      const list = await searchProjectQuotes(projectId, searchQuery);
      setQuoteList(list);
      if (draft.quoteNumber === quoteNumber) {
        await handleNewQuote();
      }
    } catch (err) {
      setDeleteError(err instanceof Error ? err.message : "삭제에 실패했습니다.");
    } finally {
      setDeletingQuoteNumber(null);
    }
  }
  ...
  async function handleExportPng() {
    ...
    try {
      ...
      // 날짜·일련번호는 파일명 맨 뒤로 보낸다(2026-09-15) — 고객사·상품명이 먼저
      // 보여야 폴더에서 찾기 쉽다는 피드백. 리비전 번호도 맨 끝에 붙인다(2026-09-18) —
      // 같은 견적을 여러 번 고쳐 저장하면 파일명이 같아서 리비전끼리 구분이 안 됐다.
      const numberPart = (draft.quoteNumber || "").replace(/^QT-/, "");
      const revLabel = `Rev${String(revision || 1).padStart(2, "0")}`;
      a.href = url;
      a.download = `견적서-${safe(draft.customerName || "고객사")}-${safe(
        productName || "상품"
      )}-${safe(numberPart)}_${revLabel}.png`;
      ...
      <div className="w-[440px] shrink-0 rounded-xl border border-neutral-200 bg-white p-5">
        ...
        {draft.id && (
          <div className="mb-3 -mt-1">
            ...
            >
              {isSavingCustomerInfo ? "저장 중..." : "고객 정보만 수정 저장 (리비전 생성 없음)"}
            </button>
            {customerInfoHint && <p className="mt-1 text-[11px] text-neutral-500">{customerInfoHint}</p>}
          </div>
        )}
        ...
        <div className="mb-3 flex gap-2">
          <div className="flex-1">
            <div className="mb-1 flex items-center justify-between">
              <label className="block text-xs font-medium text-neutral-700">견적일자</label>
              <label className="flex items-center gap-1 text-[11px] text-neutral-500">
                <input
                  type="checkbox"
                  className="h-3.5 w-3.5 rounded border-neutral-300"
                  checked={draft.quoteDate !== ""}
                  onChange={(e) =>
                    set("quoteDate", e.target.checked ? new Date().toISOString().slice(0, 10) : "")
                  }
                />
                표시
              </label>
            ...
            >
              ...
              <span className="flex shrink-0 gap-1">
                ...
                <button
                  ...
                  disabled={deletingQuoteNumber === q.quoteNumber}
        ...
        {deleteError && <p className="mt-1 rounded-md bg-red-50 px-2 py-1.5 text-[11px] text-red-600">{deleteError}</p>}
