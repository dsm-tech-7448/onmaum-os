...
declare module "popbill" {
  ...
  export interface TaxinvoiceServiceInstance {
    ...
    // 이미 발행된 세금계산서를 원하는 주소로 (재)발송한다 — registIssue 시점의
    // invoiceeEmail1과 별개로, 발행 이후 언제든 호출 가능하다.
    sendEmail(
      corpNum: string,
      keyType: "SELL" | "BUY" | "TRUSTEE",
      mgtKey: string,
      receiver: string,
      onSuccess: (response: { code: number; message: string }) => void,
      onError: (error: PopbillError) => void
    ): void;
  }
