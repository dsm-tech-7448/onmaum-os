...
export const quotes = pgTable(
  "quotes",
  {
    ...
    quoteDate: date("quote_date"), // 견적일자 — 고객이 날짜 표기를 빼달라고 하는 경우가 있어 선택 입력(2026-09-22)
...
export const poItems = pgTable("po_items", {
  ...
  // 원본 견적 품목 — 한 발주서에 여러 견적 품목을 함께 담을 수 있게 되면서(2026-09-15,
  // 같은 공급업체로 보내는 여러 품목을 발주서 하나로 묶는 기능) purchase_orders.quote_item_id
  // 하나로는 그중 첫 품목만 가리킬 수 있어, 품목 줄 각각이 자기 견적 품목을 알 수 있도록
  // 추가한다. statement_items.quote_item_id와 동일한 패턴. 이 값이 비어있는 기존 발주
  // 품목(이 필드가 생기기 전 데이터)은 Summary 집계에서 발주서의 quote_item_id로 대체
  // 매칭한다(품목이 1줄뿐인 발주서에 한해서만 — 여러 줄이면 어느 품목인지 알 수 없어 매칭하지 않음).
  quoteItemId: uuid("quote_item_id").references(() => quoteItems.id),
...
export const taxInvoices = pgTable("tax_invoices", {
  ...
  customerBusinessNumber: varchar("customer_business_number", { length: 50 }),
  // 종사업장번호 — 한 사업자등록번호 아래 여러 사업장(지점)이 있을 때만 있는 4자리 값,
  // 대부분은 비어 있다(2026-09-22). 팝빌 발행 시 invoiceeTaxRegID로 그대로 전달한다.
  customerSubNum: varchar("customer_sub_num", { length: 10 }),
  ...
  // 공급받는자 이메일 — 입력해두면 발행(registIssue) 시 invoiceeEmail1로 함께 전달돼
  // 문서 자체에 등록된다(2026-09-22). "이메일로 발송" 액션은 이 값과 별개로 언제든
  // 다른 주소로도 보낼 수 있는 부가 기능 — 문서에 등록된 이메일이 있어야 팝빌에서
  // 문서 조회 시에도 이메일이 "확인"된다(발송 이력에만 남고 문서 정보엔 안 남던 문제,
  // 2026-09-22 첫 실제 발행 TI-20260922-0011에서 발견).
  customerEmail: varchar("customer_email", { length: 200 }),
  // "이메일로 발송" 성공 시 기록(2026-09-25) — 이미 발행 완료된 문서는 customerEmail을
  // 소급 등록할 수 없어서(팝빌 문서 자체엔 영영 이메일이 안 남음), 실제로 보낸 적이
  // 있다는 걸 우리 앱에서라도 확인할 수 있게 최근 발송 주소/시각을 별도로 남긴다.
  lastEmailSentTo: varchar("last_email_sent_to", { length: 200 }),
  ...
  // 작성일자 — 세금계산서에 인쇄/신고되는 공급시기 기준 날짜. 실제로 "발행 완료 처리"를
  // 누르는 날짜(발행일자/전송일자)와 다를 수 있다(예: 월말 납품, 발행은 다음 달 초에
  // 처리) — registIssue의 writeDate로 이 값을 그대로 전달한다(2026-09-25, 이전에는
  // registIssue 호출 시점의 오늘 날짜로 하드코딩돼 있었다).
  scheduledDate: date("scheduled_date"), // 작성일자(구 "발행예정일")
  // 영수/청구 — 이미 대금을 받았으면 "영수", 아직 안 받고 청구하는 거면 "청구"(기본값).
  // 팝빌 발행(registIssue)의 purposeType 파라미터로 그대로 전달된다(2026-09-22, 예전에는
  // "청구"로 하드코딩하고 "계약 조건에 따라 영수로 바꿔야 할 수 있음"이라는 메모만
  // 남겨뒀었다 — src/lib/popbill/tax-invoice.ts 참고).
  purposeType: varchar("purpose_type", { length: 10 }).notNull().default("청구"), // 영수 | 청구
...
export const taxInvoiceItems = pgTable("tax_invoice_items", {
  ...
  qty: numeric("qty", { precision: 12, scale: 2 }),
  price: numeric("price", { precision: 12, scale: 2 }),
  supplyOverride: numeric("supply_override", { precision: 12, scale: 2 }), // 공급가액(총) — 2026-09-22부터 세금계산서는 품목을 한 줄로 합쳐 이 값에 총 공급가액을 직접 입력한다(단가는 더 안 씀)
  taxOverride: numeric("tax_override", { precision: 12, scale: 2 }), // 세액(총) — 비우면 공급가액의 10%로 자동 계산(2026-09-22)
  sortOrder: integer("sort_order").notNull(),
});
 ...
 * quote_id는 선택(nullable) — 한 프로젝트에 서로 다른 고객의 견적이 여러 건 걸릴 수
 * 있는데(2026-09-15, Summary 화면에서 "박종현 건 요청사항이 다른 고객 행에도 같이
 * 보인다"는 피드백), Summary 화면에서 특정 품목 행에 추가한 요청은 이 값을 채워 그
 * 견적에만 보이게 한다. 프로젝트 상세 화면 자체의 요청사항 위젯처럼 특정 견적과
 * 무관한 진짜 프로젝트 전체 메모는 비워둔 채로 남겨 모든 행에 계속 보이게 둔다.
 */
...
export const summaryOverrides = pgTable("summary_overrides", {
  ...
  qty: numeric("qty", { precision: 12, scale: 2 }),
  reqDate: date("req_date"), // 발주서 발송요청일 정정값 — 대시보드 발송 지연 알림이 참조(queries.ts)
  // 고객이 특별히 요청한 납품 희망일 — 발송요청일(reqDate, 공급업체용)과는 별개 개념
  // (2026-09-15, "납기에 표시된 날짜는 내가 입력한 게 아니다" 피드백). Summary에서만
  // 수기로 입력하고, 같은 견적의 모든 품목 행에 같은 값이 적용된다.
  custReqDate: date("cust_req_date"),
  supplierName: varchar("supplier_name", { length: 200 }),
  supplierPhone: varchar("supplier_phone", { length: 30 }),
  shipDate: date("ship_date"),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
...
/**
 * "Daily Summary" 화면(/dashboard/daily-summary) 전용 테이블 — 원본 Daily Summary 엑셀
 * (D:\0. 회사\0. Daily Summary- 0323(New).xlsx)의 컬럼을 한 칸도 빠짐없이 그대로 옮긴다.
 * historical_orders(/dashboard/summary용, 컬럼이 축소된 요약 이력)와는 별개 — 이 테이블은
 * 그 화면과 완전히 분리된, 엑셀과 1:1 대응하는 원본 그대로의 기록이다.
 *
 * 행 출처는 두 가지뿐이다: source="excel"(엑셀 재업로드로 채워짐, scripts/import-daily-summary-full.ts)
 * 또는 source="manual"(화면에서 사용자가 직접 입력). 둘 다 어떤 견적서/발주서/거래명세서에도
 * 연결되지 않은 "보낸 문서가 아닌" 순수 기록이라 — summary_overrides 같은 그림자 테이블 없이
 * 모든 칸을 바로 UPDATE해도 안전하다(quotes/purchase_orders와 달리 고객·공급업체에 실제로
 * 발송된 문서가 아니므로).
 */
export const dailySummaryEntries = pgTable("daily_summary_entries", {
  id: uuid("id").primaryKey().defaultRandom(),
  source: varchar("source", { length: 20 }).notNull(), // excel | manual
  division: varchar("division", { length: 50 }), // 구분
  orderDate: date("order_date"), // 날짜
  contactName: varchar("contact_name", { length: 100 }), // 이름
  customerName: varchar("customer_name", { length: 200 }), // 기관명
  contactOffice: varchar("contact_office", { length: 50 }), // 연락처-Office
  contactMobile: varchar("contact_mobile", { length: 50 }), // 연락처-Mobile
  productCode: varchar("product_code", { length: 100 }), // 제품코드
  productName: varchar("product_name", { length: 300 }), // 제품명
  qty: numeric("qty", { precision: 12, scale: 2 }), // 필요수량
  reqDate: date("req_date"), // 요청날짜
  supplierName: varchar("supplier_name", { length: 200 }), // 공급업체
  supplierPhone: varchar("supplier_phone", { length: 50 }), // 전화번호(공급업체)
  shipDate: date("ship_date"), // 출고날짜
  note: text("note"), // 특기사항
  paidCash: numeric("paid_cash", { precision: 14, scale: 2 }), // 결제-현금
  paidCard: numeric("paid_card", { precision: 14, scale: 2 }), // 결제-카드
  outAmount: numeric("out_amount", { precision: 14, scale: 2 }), // 출금
  cashReceiptDate: date("cash_receipt_date"), // 영수증-현금영수증
  taxInvoiceDate: date("tax_invoice_date"), // 영수증-세금계산서
  sourceFile: varchar("source_file", { length: 100 }), // 엑셀 이관 시 출처 파일명(디버깅용), manual이면 null
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  updatedBy: uuid("updated_by").references(() => users.id),
});

export type DailySummaryEntry = typeof dailySummaryEntries.$inferSelect;
export type NewDailySummaryEntry = typeof dailySummaryEntries.$inferInsert;
